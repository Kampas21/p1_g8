...
class PedidoService
{
        ...
        try {
            ...
            foreach ($lineas as $producto_id => $item) {
                ...
                for ($i = 0; $i < $cantidad; $i++) {
                    // Como tu compañero ha modificado cómo se añaden productos, llamamos a la función adaptada
                    self::addProducto($pedido_id, (int) $producto_id, $precio_unitario);
                }
            }
    ...
    {
        // Fusionamos la confirmación base (DAO) con la liquidación de Bistrocoins
        $ok = PedidoDAO::confirmarPedido($pedido_id, $metodo_pago, $total);
    ...
    {
        // Fusionamos el cambio de estado base con la lógica de recompensas
        $ok = PedidoDAO::cambiarEstado($pedido_id, $estado_nuevo);
    ...
    public static function getPedidosActivosByUsuario(int $usuario_id): array
    {
        global $conn;
        $sql = "
            SELECT numero_pedido, estado, fecha_hora, total, id
            FROM pedidos
            WHERE usuario_id = ?
